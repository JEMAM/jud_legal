...
export default function TriagemPage() {
  ...
  // Busca de CPF / CNPJ na Web (Google)
  const [consultaNome, setConsultaNome] = useState("");
  const [docLocalizado, setDocLocalizado] = useState("");
  const [buscandoDoc, setBuscandoDoc] = useState(false);
  const [docMsg, setDocMsg] = useState("");

  const handleBuscarDoc = async () => {
    if (!consultaNome.trim()) return;
    setBuscandoDoc(true);
    setDocMsg("Buscando no Google/Web...");
    try {
      const res = await fetch(getApiUrl("/api/buscar-documento"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome_razao: consultaNome }),
      });
      if (res.ok) {
        const data = await res.json();
        setDocLocalizado(data.documento);
        setDocMsg(data.encontrado ? "Documento localizado!" : "Nenhum CPF/CNPJ público encontrado.");
      } else {
        setDocMsg("Erro ao consultar documento.");
      }
    } catch (err) {
      setDocMsg("Falha na conexão com o servidor.");
    } finally {
            ...
            <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="bg-slate-950/60 border border-slate-800/80 rounded-xl p-3.5 space-y-3">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="text-slate-300 dark:text-slate-200 text-xs font-bold uppercase tracking-wider block mb-1 flex items-center justify-between">
                      <span>Nome / Razão Social</span>
                      <span className="text-xxs font-semibold text-indigo-600 dark:text-indigo-300 bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-500/20 uppercase">busca web (google)</span>
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        placeholder="Ex: Petrobras, Banco do Brasil, Nome..."
                        value={consultaNome}
                        onChange={(e) => setConsultaNome(e.target.value)}
                        onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); handleBuscarDoc(); } }}
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500 transition-colors"
                      />
                      <button
                        type="button"
                        onClick={handleBuscarDoc}
                        disabled={buscandoDoc || !consultaNome.trim()}
                        className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white text-xs font-bold px-3.5 py-2 rounded-xl transition-all flex items-center gap-1.5 flex-shrink-0 cursor-pointer shadow-lg shadow-indigo-600/20"
                      >
                        {buscandoDoc ? <span className="animate-spin">⏳</span> : <Search className="h-3.5 w-3.5" />}
                        <span>Buscar</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div>
                    <label className="text-slate-300 dark:text-slate-200 text-xs font-bold uppercase tracking-wider block mb-1 flex items-center justify-between">
                      <span>CPF / CNPJ Localizado</span>
                      {docMsg && <span className="text-xxs font-bold text-emerald-600 dark:text-emerald-300 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/20 uppercase truncate max-w-[170px]">{docMsg}</span>}
                    </label>
                    <div className="flex gap-2">
                      <input
                        type="text"
                        ...
                        value={docLocalizado}
                        className="w-full bg-slate-900 border border-slate-750 rounded-xl px-3 py-2 text-sm text-indigo-700 dark:text-indigo-300 font-mono font-bold focus:outline-none"
                      />
                      <button
                        type="button"
                        onClick={() => {
                          if (consultaNome.trim()) {
                            setNomeParte(consultaNome.trim());
                          }
                        }}
                        disabled={!consultaNome.trim()}
                        title="Usar este nome no campo Nome da Parte para consulta no PJe"
                        className="bg-slate-800 hover:bg-slate-750 disabled:opacity-40 text-slate-100 dark:text-white text-xs font-semibold px-3 py-2 rounded-xl transition-all flex items-center gap-1 flex-shrink-0 cursor-pointer shadow-sm"
                      >
                        <Copy className="h-3.5 w-3.5" />
                        <span className="hidden sm:inline">Usar no PJe</span>
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
                <label className="flex items-center gap-2.5 cursor-pointer bg-slate-950 border border-slate-850 px-4 py-3 rounded-xl hover:bg-slate-950/80 transition-all h-[46px]">
                  ...
                  <div className="flex flex-col">
                    <span className="text-sm font-semibold text-slate-800 dark:text-slate-200">Apenas Processos Monitorados</span>
                    <span className="text-xxs text-slate-600 dark:text-slate-400">Limitar busca a processos do banco</span>
                  </div>
                </label>

                <div className="relative" ref={dropdownRef}>
                  <label className="text-slate-300 dark:text-slate-200 text-xs font-bold uppercase tracking-wider block mb-1">Processos Monitorados no Filtro</label>
                  <button
                    type="button"
                    ...
                    className={`w-full border px-3 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-between gap-2 transition-all ${!apenasMonitorados
                      ? "bg-slate-900/40 border-slate-850 text-slate-600 dark:text-slate-400 cursor-not-allowed"
                      : "bg-slate-950 border-slate-800 hover:border-slate-750 text-slate-800 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-white"
                      }`}
                  >
